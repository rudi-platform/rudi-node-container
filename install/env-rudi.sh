...
PORTAL_REQ_TIMEOUT=${PORTAL_REQ_TIMEOUT:-5000}
PORTAL_MAX_RETRIES=${PORTAL_MAX_RETRIES:-3}
PORTAL_INITIAL_DELAY=${PORTAL_INITIAL_DELAY:-500}
...
#
# Migration switch
CATALOG_AUTO_UPDATE_SCHEMA=${CATALOG_AUTO_UPDATE_SCHEMA:-true}

#
# Extra
DB_WAIT_MAX=${DB_WAIT_MAX:-60}
LOG_ROTATE_CONF=${LOG_ROTATE_CONF:-${INI_DIR}/log_rotate}
GLOBAL_VARS="APP_DIR INI_DIR SAFE_DIR MEDIA_DIR LOG_DIR BACKUP_DIR MONGODB DB_PREFIX PUBKEY_DIR TRUSTED_DOMAINS"
GLOBAL_VARS="${GLOBAL_VARS} PORTAL_URL PORTAL_USER PORTAL_PASS PORTAL_REQ_TIMEOUT PORTAL_MAX_RETRIES PORTAL_INITIAL_DELAY"
GLOBAL_VARS="${GLOBAL_VARS} JWTAUTH_PORT JWTAUTH_LOCAL_URL LOCAL_INTERFACE LOCAL_URL"
GLOBAL_VARS="${GLOBAL_VARS} CATALOG_PORT CATALOG_LOCAL_URL CATALOG_PREFIX CATALOG_PUBLIC_URL CATALOG_AUTO_UPDATE_SCHEMA"
...
rudi_check() {
    ...
    dirCheck ${BACKUP_DIR}          || error "Could not access or create ${BACKUP_DIR}"

    chown $user:$group ${INI_DIR} ${SAFE_DIR} ${LOG_DIR} ${DB_DIR} ${BACKUP_DIR}
    chmod 770          ${INI_DIR} ${SAFE_DIR} ${LOG_DIR} ${DB_DIR} ${BACKUP_DIR}
